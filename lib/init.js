...
const fs = require("fs")
const lerna = require("./configs/json/lerna")
const npm = require("./configs/json/npm")
const shared = require("./configs/json/shared")
...
module.exports = {
  ...
  run: async function () {
    try {
      ...
      fs.mkdir("./packages/shared", (err) => {
        if (err) {
          console.log(err)
        }
      })
      // create shared package with package.json
      fs.writeFile("./packages/shared/package.json", "", () => null)
      fs.appendFile(
